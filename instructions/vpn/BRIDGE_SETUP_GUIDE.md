...
Клиент (телефон/ПК) → **TrustTunnel** (протокол на базе HTTPS/QUIC) → **Российский VPS (Мост)** → **VLESS+REALITY (Xray)** → **Зарубежный VPS (Цель)** → Интернет
...
Пример `ExecStart` в юните (без использования флагов):
...
2. Внешний IP должен соответствовать **IP Зарубежного VPS**.

---

## 6. Резюме

Схема обеспечивает высокую скрытность и низкую задержку. TrustTunnel выступает в роли надежного транспорта, а Xray обеспечивает обход блокировок на внешнем контуре.
